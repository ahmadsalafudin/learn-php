...
$header = array('No.', 'Tanggal','NIK', 'Nama Karyawan', 'Pinjaman(Rp)', 'Status');

// Load Data dari Database
$dataku = mysql_query("SELECT pinjaman.*, karyawan.nik, karyawan.nm_karyawan FROM pinjaman
								LEFT JOIN karyawan ON pinjaman.kd_karyawan=karyawan.kd_karyawan
								ORDER BY pinjaman.no_pinjaman ASC LIMIT");
$no = 0;
	while ($tampil=mysql_fetch_array($dataku)){
	$no++;
	// Simpan Kedalam Array dengan Batasan |
	@$gue[] .= $no."|".$tampil['tanggal']."|".$tampil['nik']."|".$tampil['nm_karyawan']."|".$tampil['besar_pinjaman']."|".$tampil['status_lunas'];
	 }
	 // Cetak Laporan
